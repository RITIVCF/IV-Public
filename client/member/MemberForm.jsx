    ...
    render() {
      return (
        ...
        <form className="publicForm" onSubmit={this.addMember.bind(this)}>
          ...
          <select ref="gradterm">
            {this.props.subscriptions.options.ready() ? this.getGradTerms().map( (term)=>{
          ...
          <label>Campus Affiliations:</label>
            {this.props.subscriptions.options.ready() ? this.getAffiliations().map( (tag)=>{
              return <li key={tag}><label>{tag}: <input type="checkbox" ref={"affiliations."+tag} name={tag} />
            </label></li>
            }) :<div></div> }
            <br/>
          <label>Community Involvement:</label>
            {this.props.subscriptions.options.ready() ? this.getCommunityLife().map( (tag)=>{
              return <li key={tag}><label>{tag}: <input type="checkbox" ref={"communitylife."+tag} name={tag} />
            </label></li>
            }) :<div></div>}
